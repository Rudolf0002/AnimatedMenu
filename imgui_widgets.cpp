...
#include "examples/example_win32_directx11/Icons.h"
...
ImVec2 ImGui::TextButton(const char* text, ...)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return { };
    ...
    va_start(args, text);
    const char* text_end;
    ImFormatStringToTempBufferV(&text, &text_end, text, args);
    va_end(args);

    ImGuiTextFlags flags = ImGuiTextFlags_NoWidthForLargeClippedText;
...
ImVec2 Add_ImVec2(ImVec2 a, ImVec2 b)
{
    return ImVec2(a.x + b.x, a.y + b.y);
}

ImVec2 Minus_ImVec2(ImVec2 a, ImVec2 b)
{
    return ImVec2(a.x - b.x, a.y - b.y);
}
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    //ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, IM_COL32(18, 18, 18, 255), true, style.FrameRounding);

    if (*k != 0 && g.ActiveId != id) {
        strcpy_s(buf_display, ("[" + std::string(KeyNames[*k]) + "]").c_str());
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(3.f, 0.f);
    ...
    RenderFrame(check_bb.Min, check_bb.Max, (*v) ? ImColor(59, 158, 233, 255) : ImColor(75, 75, 75, 255), true, !(*v), 5.f, 1.5f);
    ...
    {
        const float pad = square_sz / 4;//ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), ImColor(0, 1, 17, 255), square_sz * 0.5f);
        //RenderFrame()
    }
...
bool ImGui::Toggle(const char* label, bool* v)
{
    ...
    float height = g.FontSize + g.Style.FramePadding.y * 1.0f;//ImGui::GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos + ImVec2(3.f, 0);
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(75, 75, 75, 255), true, true, height * 0.5f, 2.f);
    RenderNavHighlight(total_bb, id);
    ...
    if (*v)
        window->DrawList->AddCircleFilled(ImVec2(pos.x + radius + t * (width - radius * 2.0f) - 2.f, pos.y + height / 2), radius - 1.5f, ImColor(59, 158, 233, 255), 36);
    else
        window->DrawList->AddCircleFilled(ImVec2(pos.x + radius + t * (width - radius * 2.0f) + 2.f, pos.y + height / 2), radius - 1.5f, ImColor(75, 75, 75, 255), 36);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), ImColor(40, 40, 40, 255), /*style.FrameRounding*/5.f, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, ImColor(40, 40, 40, 255), /*style.FrameRounding*/5.f, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderText(ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y + 1.5f), popup_open ? ICON_FA_ARROW_UP : ICON_FA_ARROW_DOWN);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, GetFontSize()), window->DC.CursorPos + ImVec2(ImGui::GetContentRegionAvail().x - 135, GetFontSize()) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (*(int*)p_data > *(int*)p_min)
        RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Max.x - 14, grab_bb.Max.y - 10), ImColor(59, 158, 233, 255), true, g.Style.FrameRounding);
    RenderFrame(ImVec2(grab_bb.Max.x - 2.f, grab_bb.Max.y - 12), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 12), ImColor(39, 39, 39, 255), true, g.Style.FrameRounding);


    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5.05f, grab_bb.Max.y - 11.f), 7, ImColor(59, 59, 59, 255), 100);
    ...
    //RenderTextClipped(ImVec2(frame_bb.Max.x - frame_bb.Min.x + 10.f, frame_bb.Min.y - GetFontSize() * 2.5f), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    RenderText(ImVec2(frame_bb.Max.x - 25.f, frame_bb.Min.y - GetFontSize()), value_buf);

    if (label_size.x > 0.0f)
    {
        RenderText(ImVec2(frame_bb.Min.x + 5.f, frame_bb.Min.y - GetFontSize()), label);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(40, GetFontSize() + g.Style.ItemInnerSpacing.y)))
        {
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
    ...
    draw_list->AddLine(pos, pos + ImVec2(half_sz.x * 4, 0), ImColor(255, 255, 255, alpha8), 2.f);
    /*
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_width = 10.f;//square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    ...
    float sv_cursor_rad = value_changed_sv ? 5.0f : 3.0f;
    ...
    if (alpha_bar)
    {
        ...
        //draw_list->AddLine(GetWindowPos() + ImVec2(bar1_pos_x - 1, bar1_line_y), GetWindowPos() + ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), ImColor(255, 255, 255, (int)style.Alpha), 2.f);
